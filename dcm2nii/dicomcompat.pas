...
procedure read_dicom_data_compat(lReadJPEGtables,lVerboseRead,lAutoDECAT7,lReadECAToffsetTables,lAutodetectInterfile,lAutoDetectGenesis,lReadColorTables: boolean; var lDICOMdata: DICOMdata; var lDTIra: TDTIRA; var lHdrOK, lImageFormatOK: boolean; var lDynStr: string;var lFileName: string; var lPrefs: TPrefs);
  ...
  Clear_Dicom_Data(lDicomData);
     ...
     if (lImgHdr = 0) and (lDicomData.ImageStart = 8432) then begin
		...
		if (not lGEodd) and (lExamHdr <> 0) then begin
		   lHdrOffset := swap32i(linitialoffset+132);//x132- int ptr to exam header
                   dcmMsg('exam header offset '+inttostr(lHdrOffset));
                   //Patient ID
...
//Read series header
		   ...
		   if lSeriesOffset > 12 then begin
			  ...
			  lDicomData.SeriesNum := lI;
                   ...
                   //image data
                   lHdrOffset := swap32i(linitialoffset+148);//x148- int ptr to image header
        end;
        if lGEodd then lHdrOffset := 2158+28;
        if ((lHdrOffset +58) < FileSz) and (lImgHdr <> 0) then begin
           dcmMsg('image header offset '+inttostr(lHdrOffset));
           ...
           lDicomData.XYZmm[3] := fswap4r (lHdrOffset+26);// slice thickness mm
           lDicomData.XYZmm[1] := fswap4r (lHdrOffset+50);// pixel size- X
           lDicomData.XYZmm[2] := fswap4r (lHdrOffset+54);//pixel size - Y
           lSliceThick := lDicomData.XYZmm[3];
           lGap :=  fswap4r (lHdrOffset+118);//1410 gap thickness mm
           if lGap > 0 then
                  lDicomData.XYZmm[3] := lDicomData.XYZmm[3] + lGap;
         ...
         //Therefore, we only use this as a last resort! on Philips, prefer to use 2001,1003+ 2005,10b0/2005,10b1/2005,10b2
                ...
                if (lUse00189117 = kUndefined) then
                 ...
                 if  (lUse00189117 = kYes) then begin //Overly complicated due to Philips shameful use of these tags
                    if lDICOMdata.nDTIdir < kMaxDTIdir then //kMaxDTIdir then
                       inc(lDICOMdata.nDTIdir);
                    //Philips may not report vectors for B0 images - so we will initialize these values
                    lDTIra[lDICOMdata.nDTIdir].bval := 0;
                    lDTIra[lDICOMdata.nDTIdir].v1 := 0;
                    lDTIra[lDICOMdata.nDTIdir].v2 := 0;
                    lDTIra[lDICOMdata.nDTIdir].v3 := 0;
                 end;
            end;//9117
          $9087: begin
                             ...
                             if {(lGrpStr = 'FD') and} (lUse00189117 <> kNo) then begin
                                ...
                                lDICOMdata.DTI.bval := round(read64 (fp,lrOK));
                                if lDICOMdata.nDTIdir < 1 then lDICOMdata.nDTIdir := 1;
                                lDTIra[lDICOMdata.nDTIdir].bval := lDICOMdata.DTI.bval;
                                if not lrOK then goto 666;
                                e_len := 0; remaining := 0;
                ...
                $9089: begin
                  if  (lUse00189117 <> kNo) {lGrpStr = 'FD'} then begin //see lUse00189117
                             ...
                             //readfloats (fp, remaining, lDummyStr, lfloat1, lfloat2, lROK);
                             lDICOMdata.DTI.v1 := read64 (fp,lrOK);
                             if not lrOK then goto 666;
                             lDICOMdata.DTI.v2 := read64 (fp,lrOK);
                             if not lrOK then goto 666;
                             lDICOMdata.DTI.v3 := read64 (fp,lrOK);
                             if not lrOK then goto 666;
                             if lDICOMdata.nDTIdir < 1 then lDICOMdata.nDTIdir := 1;
                             lDTIra[lDICOMdata.nDTIdir].v1 := lDICOMdata.DTI.v1;
                             lDTIra[lDICOMdata.nDTIdir].v2 := lDICOMdata.DTI.v2;
                             lDTIra[lDICOMdata.nDTIdir].v3 := lDICOMdata.DTI.v3;
           ...
           case element of //1362
             ...
             $1028: begin   //7/2013
                ...
                $000C,$100C: begin
                             ...
                             lDICOMdata.DTI.bval := round(lFloat1);
                      ...
                      $1003: begin //bvalue  - see lUse00189117
                            ...
                            if (e_len = 4) {and (lUse00189087 = kNo)} then begin
                               if (lDICOMdata.nDTIdir < kMaxDTIDir) and (not (lUse00189117 = kYes)) then //see 0018,9117 - Philips' usage shameful
                                  inc(lDICOMdata.nDTIdir);
                               lUse00189117 := kNo;
                               lDICOMdata.DTI.bval := round(read32r(fp,lrOK));
                               lDTIra[lDICOMdata.nDTIdir].bval := lDICOMdata.DTI.bval;
		...
		case element of
            ...
            $10b0: begin //see lUse00189117
                ...
                if (e_len = 4) and  (lUse00189117 = kNo) then begin
                   lDICOMdata.DTI.v1 := read32r(fp,lrOK);
                   if lDICOMdata.nDTIdir < 1 then lDICOMdata.nDTIdir := 1;
                   lDTIra[lDICOMdata.nDTIdir].v1 := lDICOMdata.DTI.v1;
                   TmpStr := floattostr(lDICOMdata.DTI.v1);
				   t := _string;
             ...
             $10b1: begin  //see lUse00189117
                if (e_len = 4) and  (lUse00189117 = kNo) then begin
                   lDICOMdata.DTI.v2 := read32r(fp,lrOK);
                   if (lDICOMdata.nDTIdir < 1) then lDICOMdata.nDTIdir := 1;
                   lDTIra[lDICOMdata.nDTIdir].v2 := lDICOMdata.DTI.v2;
				   TmpStr := floattostr(lDICOMdata.DTI.v2);
				   t := _string;
             ...
             $10b2: begin   //see lUse00189117
                if (e_len = 4) and (lUse00189117 = kNo) then begin
                   lDICOMdata.DTI.v3 := read32r(fp,lrOK);
                   if (lDICOMdata.nDTIdir < 1) then lDICOMdata.nDTIdir := 1;
                   lDTIra[lDICOMdata.nDTIdir].v3 := lDICOMdata.DTI.v3;
				   TmpStr := floattostr(lDICOMdata.DTI.v3);
				   t := _string;
...
if (lPrefs.PhilipsPrecise) and (lManufacturerIsPhilips) and (lPhilipsScaleSlope <> 0) then begin
   //if (true) and (lManufacturerIsPhilips) and (lPhilipsScaleSlope <> 0) then begin   //66666666
   //dcmMsg(floattostr(lDICOMdata.IntenScale)+' '+floattostr(lDICOMdata.intenIntercept) +'  '+floattostr(lPhilipsScaleSlope));
...
if (lDICOMdata.ManufacturerID = kPhilipsID) and (lDICOMdata.nDTIdir > 1) then begin
   ...
   for i := 1 to lDICOMdata.nDTIdir do
       if lDTIra[i].bval <> lDICOMdata.DTI.bval then
          lGELX := false;//multiple B0 directions
   if lGELX then
      lDICOMdata.nDTIdir := 1; //only report multiple dti directions if there is variability in the diffusion values
   lGELX := false;
end;
...
if lVerboseRead then begin
           ...
           dcmMsg ('DTI bvalue:'+ inttostr(lDICOMdata.DTI.bval));
           dcmMsg ('DTI bvec:'+floattostrf(lDicomData.DTI.v1,ffFixed,8,2)+'/'+floattostrf(lDicomData.DTI.v2,ffFixed,8,2)+'/'+floattostrf(lDicomData.DTI.v3,ffFixed,8,2) );
           dcmMsg ('Little-endian:'+inttostr(lDicomData.little_endian));
           dcmMsg('Image start (bytes):'+inttostr(lDicomData.ImageStart));
end;
 //dcmMsg('abba'+inttostr(lDICOMdata.CompressOffset)+'  '+inttostr(lDICOMdata.CompressSz));
  666:
