...
interface
uses nifti_types, define_types, sysutils, classes, dialogs,StrUtils;//2015! dialogsx
...
var
  ...
  mat, d, t: mat33;
  matElements, compressedDataSize, headerSize, nItems, i, channels, fileposBytes: longint;
  offset: array [0..3] of single;
  elementSize: array [0..3] of single;
...
var
  ...
  qto_xyz: mat44;
  //dumqx, dumqy, dumqz, dumdx, dumdy, dumdz: single;
begin
    nif_x_axnum := -1;
    nif_y_axnum := -1;
    nif_z_axnum := -1;
...
666:
  ...
  if (not FileExistsEX(fname)) then begin
    fname := fname+'.gz';
    gzBytes := K_gzBytes_onlyImageCompressed; //separate file, so K_gzBytes_headerAndImageCompressed would also work
  end;
end;
