...
{$ENDIF}
  ...
  RXSpin,perisettings, Menus,ClipBrd,metagraph,periplot;
...
procedure TGraph4DForm.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
{$ENDIF}
     ...
     ImgForm.YViewEdit.asInteger := gMRIcroOverlay[kBGOverlayNum].NIFTIhdr.dim[2] div 2;
     ImgForm.ZViewEdit.asInteger := gMRIcroOverlay[kBGOverlayNum].NIFTIhdr.dim[3] div 2;
...
procedure TGraph4DForm.CopyMenuClick(Sender: TObject);
...
procedure TGraph4DForm.SaveMenuClick(Sender: TObject);
begin
 if gWMF.Empty then begin
     showmessage('Please Open a dataset first.');
     exit;
...
procedure TGraph4DForm.FFTmenuClick(Sender: TObject);
begin
{$IFDEF FFTs}
 FFT4ROI (g4DHdr);
 exit;
...
begin
      if (g4DHdr.ImgBufferItems = 0) then begin
       ...
       ConvertToTrace(g4DHdr,g4Ddata,ImgForm.XViewEdit.asInteger,ImgForm.YViewEdit.asInteger,ImgForm.ZViewEdit.asInteger);
	 (*if (ssAlt in Shift) then begin
                 TextForm.Memo1.Lines.Clear;//prepare to report results
            TextToTrace (g4Ddata);
         end;*)
         Plot4DTrace(TrackBar1.position);
end;
...
procedure TGraph4DForm.BatchMenuClick(Sender: TObject);
label
     111;
var
   lStr: string;
...
function SelectDirectory(const Title: string; const Flag: integer): string;
//requires ShlObj in uses list
var
