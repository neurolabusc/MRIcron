...
procedure Mat2Hdr (var lM: TMatrix; var lHdr:  TNIFTIhdr);
...
procedure Mat2Hdr (var lM: TMatrix; var lHdr:  TNIFTIhdr);
begin
  lHdr.srow_x[0] := lM.matrix[1,1];
  lHdr.srow_x[1] := lM.matrix[1,2];
  lHdr.srow_x[2] := lM.matrix[1,3];
  lHdr.srow_x[3] := lM.matrix[1,4];

  lHdr.srow_y[0] := lM.matrix[2,1];
  lHdr.srow_y[1] := lM.matrix[2,2];
  lHdr.srow_y[2] := lM.matrix[2,3];
  lHdr.srow_y[3] := lM.matrix[2,4];


  lHdr.srow_z[0] := lM.matrix[3,1];
  lHdr.srow_z[1] := lM.matrix[3,2];
  lHdr.srow_z[2] := lM.matrix[3,3];
  lHdr.srow_z[3] := lM.matrix[3,4];
end;
  ...
  lTrilinearSmooth := lTrilinearSmoothIn;
   ...
   {$ENDIF}
    ...
    //CopyHdrMat(lTargHdr,lDestHdr);//destination has dimensions and rotations of destination
     ...
     lPos := 0;
     if (lTrilinearSmooth) and (OneToOne(lMat)) then
     ...
     for lZi := 0 to (lZ-1) do begin
         ...
         for lYi := 0 to (lY-1) do begin
             ...
             for lXi := 0 to (lX-1) do begin
                 ...
                 //need to test Xreal as -0.01 truncates to zero
                 if (lXo >= 0) and (lYo >= 0) and (lZo >= 0) and
                     (lXo < (lXs)) and (lYo < (lYs) ) and (lZo < (lZs))
                     //12/12/2012 removed -1 for nearest neighbor (lXo < (lXs -1)) and (lYo < (lYs -1) ) and (lZo < (lZs))
                  then begin
                    lOverlap := true;
                    inc(lXo);//images incremented from 1 not 0
		    lYo := lYo*lXs;
...
var
  lXrM1,lYrM1,lZrM1,lZx,lZy,lZz,lYx,lYy,lYz,lXreal,lYreal,lZreal: single;
 lXo,lYo,lZo,lMinY,lMaxY,lMinZ,lMaxZ,
 lXYZo,lPoso,lPos,lXs,lYs,lZs,lXYs,lXYZs,lX,lY,lZ,lOutVolItems,
 lXi,lYi,lZi: integer;
 lTrilinearSmooth,lOverlap: boolean;
 lDestHdr: TNIFTIhdr;
 lMat: TMatrix;
...
begin
    ...
    lMat := Voxel2Voxel (lTargHdr,lSrcHdr.NIfTIHdr);
    //fx(lMat.matrix[1][1],lMat.matrix[1][1],lMat.matrix[2][1],lMat.matrix[3][1]);
    //lDestHdr := lSrcHdr; //destination has the comments and voxel BPP of source
    //CopyHdrMat(lTargHdr,lDestHdr);//destination has dimensions and rotations of destination
     //now we can apply the transforms...
     //build lookup table - speed up inner loop
...
if lTrilinearSmooth then begin//compute trilinear interpolation
     ...
     for lZi := 0 to (lZ-1) do begin
         ...
         lZy := lZi*lMat.matrix[2][3];
         lZz := lZi*lMat.matrix[3][3];
         //fx(lZx,lZy,lZz,+lMat.matrix[3][4]);
         for lYi := 0 to (lY-1) do begin
             //these values change once per row
             // compute once per row
             lYx :=  lYi*lMat.matrix[1][2];
             ...
             for lXi := 0 to (lX-1) do begin
                 ...
                 lZo := round(lXz^[lXi]+lYz+lZz+lMat.matrix[3][4]);
                 lPoso := lXo+lYo+lZo ;
                 if (lPoso > 0) and (lPoso <= lXYZo)
                 //if (lXo >= 0)and (lYo >= 0) and (lZo >= 0)  and (lXo < (lXs -1)) and (lYo < (lYs -1) ) and (lZo < lZs)
                  then begin
                    lOverlap := true;
                    inc(lXo);//images incremented from 1 not 0
			              lYo := lYo*lXs;
     ...
     {lPos := 1;
     case  lSrcHdr.ImgBufferBPP of
          1 : while (lPos <= (lX*lY*lZ)) and (l8i^[lPos] = 0) do inc(lPos);
	        2: while (lPos <= (lX*lY*lZ)) and (l16i^[lPos] = 0) do inc(lPos);
          4: while (lPos <= (lX*lY*lZ)) and (l32f^[lPos] = 0) do inc(lPos);
     end; //case
    ...
    if lSrcHdr.ImgBufferBPP = 4 then begin
	    ...
	    lSrcHdr.ImgBuffer := ByteP($fffffff0 and (integer(lSrcHdr.ImgBufferUnaligned)+15));
	    lSrcHdr.ImgBufferItems := lOutVolItems;
	    //CopyMemory(Pointer(lSrcHdr.ImgBuffer),Pointer(l32f),(lOutVolItems*sizeof(Single)));
	    //move(Pointer(l32f),Pointer(lSrcHdr.ImgBuffer),(lOutVolItems*sizeof(Single)));
