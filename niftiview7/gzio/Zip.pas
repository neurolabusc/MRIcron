...
//  If the TZip.AddPath property is assigned, TZip will change
//  the current directory to that path (ChDir) just before Adding.

(*
Current Dll issues (ver 1.6):
  1. when adding Foldernames to FileSpecList, a trailing slash is required
     to indicate they are folders.
  2. ??bug - if filenames stored in OEM format the dll will extract but
     not delete them.
...
uses
  ...
  ZipDlls, StdCtrls, ComCtrls {$IFNDEF VER120_PLUS}, FileCtrl {$ENDIF};
...
function GetTempPathWinOS: string;
...
begin
  ...
  result := GetVolumeInformation(pchar(DrivePath),
               VolName, 12, nil, dummy1, dummy2, nil, 0 );
...
begin
  result := 0;
  //nb: cannot detect floppy disks if across the network
...
begin {LoadFileList}
  ...
  try
    ...
    if (fEocInfo.eoch.ThisDiskNo > 1000) or
       (fEocInfo.eoch.CentralDiskNo > 1000) or
       (fEocInfo.eoch.CentralSize > 1024* 1000) then
         raise ZipException.createfmt(s_not_a_zip_file,[Filename]);
...
//static variables used by ZCallback()
var
  fn: string = '';
  BatchCount: longint = 0;   // no. of files in batch
  TotalBytes: longint = 0;   // total uncompressed size of all files
  BytesCounted: longint = 0; // total bytes counted in batch
  SkippedCount: longint = 0;
...
begin
  ...
  if not TZip(ZCallBackRec.Caller).fCancel then
    with ZCallBackRec^, TZip(Caller) do
      case ActionCode of
        1: begin
             fn := msg; //save for progress event (ActionCode = 2) ...
             if assigned(fZEDBeginFileEvent) then
               fZEDBeginFileEvent(Caller, fn, Size, BatchCount, TotalBytes);
             if fShowProgress and assigned(fProgressForm) then
               TProgressForm(fProgressForm).Text := fn;
           end;

        2: //Size = size of each block just processed (usually <= 32k)
           begin
             inc(BytesCounted,Size);
             if assigned(fZEDProgressEvent) then
               fZEDProgressEvent(Caller, fn, Size, TotalBytes);
             if fShowProgress and assigned(fProgressForm) then
               TProgressForm(fProgressForm).ProgressPosition := BytesCounted;
           end;

        3: //end of a batch of ZIP/UNZIP operations.
           //todo - check that SkippedCount works
           begin
             if Assigned(fZEDEndBatchEvent) then
               fZEDEndBatchEvent(Caller, SkippedCount, BatchCount, TotalBytes);
             if fShowProgress and assigned(fProgressForm) then
               fProgressForm.hide;
           end;

        4: ; //used for debugging with breakpoint
             //msg = routine status message - ErrorCode occasionally used

        5: BatchCount := Size; //No. of Files in batch

        6: begin
             TotalBytes := Size; //Total bytes in batch
             BytesCounted := 0;  //reset
             SkippedCount := 0;  //reset
             if fShowProgress and assigned(fProgressForm) then
               TProgressForm(fProgressForm).ProgressMax := TotalBytes;
           end;
        ...
        8: //wrong or no password. (if Odd(ErrorCode) then Add operation else Extract)
           begin
             txt := '';
             //Msg = filename...
             if fShowProgress and assigned(fProgressForm) then
             ...
             txt := InputBox(application.title,
               format(s_new_password_required, [StrPas(Msg)]),txt);
             if fShowProgress and assigned(fProgressForm) then
               fProgressForm.show;
             if txt <> '' then
             begin
        ...
        11: // Extract(UnZip) was skipped and why ...
            ...
            //   FOnExtractSkipped(Caller, Msg, UnZipSkipTypes(Size));
    end;
...
begin
  ...
  begin
    application.messagebox(pchar(format(s_extract_error_no_folder,[fExtractPath])),
      pchar(application.title),mb_iconinformation);
    exit;
  end;
  ...
  if fExtractPath = '' then
    SetCurrentDir(extractfilepath(fFilename)) else
    SetCurrentDir(fExtractPath);
...
begin
   ...
   begin
      ...
      begin //ADD...
        ...
        else if not (aoWithFullPath in Owner.fAddOptions) then JunkDir := true;
...
begin
  ...
  try
    ...
    if IsAdd and (fPassword <> '') then ZipParams.ZipPassword := pchar(fPassword);
