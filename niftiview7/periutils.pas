...
var
   lOnsetText: TextFile;
   lnEvents: integer;
   lPrev,lFloat,lFloat2,lFloat3: single;
begin
  result := false;
  ...
  while not EOF(lOnsetText) do begin
      ...
      if (ioresult = 0) and (lFloat3 > 0) then begin
           inc(lnEvents);
           if lFloat < lPrev then begin
               closefile(lOnsetText);
               showmessage('Error reading 3-column file: durations should be in ascending order '+floattostr(lFloat)+'<'+floattostr(lPrev)+': '+l3ColTextFileName);
               exit;
           end;
           lPrev := lFloat;
      end; //if not io error and valid weighting
  end; //while not EOF
  //fx(lnEvents);
